...
console.log(t,r,l,r-l)
	...
	function getLabel(e,title){
		...
		else if(e.data.builder.by == "signaturepc") lbl = title+'<br />Signatures (%): '+(e.data.hexmap.data['signaturepc'][e.data.region].toFixed(2)||0);
	...
	this.loadResults = function(type){
		...
		this.hex.data[type] = {};
		if(type == "signature" || type == "signaturepc"){
	...
	this.setColours = function(type){
		...
		if(type == "signaturepc"){
			...
			key = min.toFixed(2)+'% <span style="'+makeGradient(a,b)+';width: 10em; height: 1em;opacity: 0.7;display: inline-block;margin: 0 0.25em;"></span>'+max.toFixed(2)+'% of a constituency';
		}
		if(type == "population"){
			var b = new Colour('#F9BC26');
